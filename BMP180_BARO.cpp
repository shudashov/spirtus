...
//<<destructor>>
BMP_BARO::~BMP_BARO(){/*nothing to destruct*/
}                    


int OSS = 1; // Oversampling Setting  0: single    1: 2 times    2: 4 times   3: 8 times                         
/**********************MSB      LSB******/
int ac1;           // 0xAA     0xAB
int ac2;           // 0xAC     0xAD
int ac3;           // 0xAE     0xAE
...
/** calculate centure **/
void BMP_BARO::BMP180_T_H(float BMP180_TEMP, int BMP180_PRES)
{
  temperature = bmp180GetTemperature(bmp180ReadUT());
  BMP180_TEMP = temperature*0.1;
  BMP180_PRES = (bmp180GetPressure(bmp180ReadUP()));
...
short BMP_BARO::bmp180GetTemperature(unsigned int ut)
{
  long x1, x2;
...
long BMP_BARO::bmp180GetPressure(unsigned long up)
{
...
int BMP_BARO::bmp180Read(unsigned char address)
{
  unsigned char data;
...
int BMP_BARO::bmp180ReadDate(unsigned char address)
{
...
unsigned int BMP_BARO::bmp180ReadUT()
{
  unsigned int ut;
  Wire.beginTransmission(BMP180ADD);
...
unsigned long BMP_BARO::bmp180ReadUP()
{
